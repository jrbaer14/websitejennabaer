...
percent_irish
```
I then pulled the geographic data for the counties from the American community survey and tied it to the original variables.
```{r}
...
Customizing Labels: Here I am creating a custom label for the map itself through the glue function.This way, while hovering over each county you can see the name and the percentenage of people with Irish ancestry.
```{r}

thelabel <- glue::glue("{ma_counties_withgeo$NAM} {ma_counties_withgeo$per_irish}")
...
Now I am creating a customized pop-up function, so when you run your cursor over each county, the total population and the percentage of the county that is of Irish descent will hover over it, as will the label for each.
```{r}
...
Here is the version of the map I previously described
```{r}
